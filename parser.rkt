...
(define (op/p op-list)
  (do [op <- (or/p (one-of/p op-list (lambda (a b) (eq? a (c->s b)))) void/p)] (pure (c->s op))))
(define factor/p (do [expr <- integer/p] (pure expr)))
(define (binary/p high-level/p op-list)
  (do
   ...
   [es <- (many/p (do [op <- (op/p op-list)] [e <- high-level/p] (pure (list op e))))]
   (pure (foldl (λ (op+rhs lhs)
                  (match op+rhs
                    [(list op rhs) (list op lhs rhs)]))
                e
                es))))
(define mul:div/p (binary/p factor/p '(* /)))
(define add:sub/p (binary/p mul:div/p '(+ -)))
