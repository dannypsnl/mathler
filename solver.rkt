...
(define (handle-tmp-set s p1 p2)
  (define new-s
    (cond
      [(string-contains? "+-*" (string p1))
       (set-subtract (list->set (set->list s)) (set #\+ #\- #\* #\/))]
      [(eq? p1 #\/) (set-subtract (list->set (set->list s)) (set #\+ #\- #\* #\/ #\0))]
      [(and (string-contains? "+-*/" (string p1)) (eq? p2 #\0)) (set #\+ #\- #\* #\/)]
      [else s]))
...
(define (generate-solver)
  ...
  (define (generate-solution result)
    ...
    (define p3 (handle-tmp-set s3 p2 p1))
    (define p4 (handle-tmp-set s4 p3 p2))
    (define p5 (handle-tmp-set s5 p4 p3))
    (define p6
      (if (eq? p5 #\/) (random-ref (set-remove (list->set (set->list s6)) #\0)) (random-ref s6)))
    (define solution (string p1 p2 p3 p4 p5 p6))
    (try (if (= result (calculate solution)) solution (generate-solution result))
